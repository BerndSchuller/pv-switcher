...
#
# MODBUS Client code
# adapted from https://github.com/sourceperl/pyModbusTCP
#
...
class ModbusClient:
    ...
    def __init__(self, host=None, port=None, unit_id=None, timeout=None,
        ...
        self.__last_except = EXP_NONE  # last expect code
        # set host
        if host:
    ...
    def last_error_txt(self):
        ...
        :rtype: str
        """
        return MB_ERR_TXT.get(self.__last_error, 'unknown error')
    ...
    def last_except_txt(self, verbose=False):
        ...
        if verbose:
            return EXP_DETAILS.get(self.__last_except, default_str)
        else:
            return EXP_TXT.get(self.__last_except, default_str)
    ...
    def read_coils(self, bit_addr, bit_nb=1):
        ...
        tx_buffer = self._mbus_frame(
            READ_COILS, struct.pack('>HH', bit_addr, bit_nb))
        # send request
    ...
    def read_discrete_inputs(self, bit_addr, bit_nb=1):
        ...
        # build frame
        tx_buffer = self._mbus_frame(
            READ_DISCRETE_INPUTS, struct.pack('>HH', bit_addr, bit_nb))
        # send request
        s_send = self._send_mbus(tx_buffer)
        # check error
        if not s_send:
    ...
    def read_holding_registers(self, reg_addr, reg_nb=1):
        ...
        # build frame
        tx_buffer = self._mbus_frame(
            READ_HOLDING_REGISTERS, struct.pack('>HH', reg_addr, reg_nb))
        # send request
        s_send = self._send_mbus(tx_buffer)
        # check error
        if not s_send:
    ...
    def read_input_registers(self, reg_addr, reg_nb=1):
        ...
        # build frame
        tx_buffer = self._mbus_frame(
            READ_INPUT_REGISTERS, struct.pack('>HH', reg_addr, reg_nb))
        # send request
        s_send = self._send_mbus(tx_buffer)
        # check error
        if not s_send:
    ...
    def write_single_coil(self, bit_addr, bit_value):
        ...
        tx_buffer = self._mbus_frame(
            WRITE_SINGLE_COIL, struct.pack('>HBB', bit_addr, bit_value, 0))
        # send request
        s_send = self._send_mbus(tx_buffer)
        # check error
        if not s_send:
    ...
    def _send(self, data):
        ...
        # check link
        if self.__sock is None:
            self.__last_error = MB_SOCK_CLOSE_ERR
